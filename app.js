/* ============================================================
   ControlPuerta — lógica principal (roles conductor / fábrica)
   ============================================================ */
const App = (function () {
  const C = window.CONFIG;
  const PLANTA = C.PLANTA;
  let role = null;            // 'conductor' | 'fabrica'
  let data = [];             // registros en memoria (se refresca en vivo)
  let tab = "puerta";        // pestaña activa de fábrica
  let myId = null;           // id del registro del conductor actual
  let cStep = "inicio";      // inicio | datos | enviado
  let cGPS = null;           // {lat,lng,acc}
  ...
  /* intervalos calculados de un registro */
  function tiempos(r) {
    return {
      espera: diffMin(r.t_puerta, r.t_ingreso),   // espera en puerta
      planta: diffMin(r.t_ingreso, r.t_salida),   // tiempo dentro
      ciclo: diffMin(r.t_puerta, r.t_salida)      // ciclo total
    };
  }
  ...
  async function boot() {
    ...
    el("gateMode").innerHTML = Store.mode === "supabase"
      ? '🟢 Conectado a Supabase · sincroniza entre celulares'
      : '🟡 Modo local (prueba) · configura Supabase para sincronizar entre celulares';
    // restaura sesión de rol
    ...
    else if (savedRole === "fabrica") enterRole("fabrica", true);
    // reloj status bar
  ...
  async function refresh() {
    ...
    updateConn();
    if (role === "fabrica") renderFabrica();
    if (role === "conductor") renderConductor();
  }
  ...
  function enterRole(r, silent) {
    ...
    el("viewConductor").hidden = r !== "conductor";
    el("viewFabrica").hidden = r !== "fabrica";
    el("roleLabel").textContent = r === "conductor" ? "Conductor" : "Fábrica / Portería";
    updateConn();
    if (r === "conductor") { cStep = myId ? "enviado" : "inicio"; renderConductor(); }
    else { renderFabrica(); }
  }
  function exitRole() {
    role = null;
    ...
    el("viewConductor").hidden = true;
    el("viewFabrica").hidden = true;
    el("gate").hidden = false;
  }
  function askPin() { el("pinOverlay").classList.add("show"); el("pinErr").hidden = true; el("pinInput").value = ""; setTimeout(() => el("pinInput").focus(), 100); }
  function closePin() { el("pinOverlay").classList.remove("show"); }
  function checkPin() {
    if (el("pinInput").value === String(C.PIN_FABRICA)) { closePin(); enterRole("fabrica"); }
    else { el("pinErr").hidden = false; }
  }

  /* ============================================================
     VISTA CONDUCTOR  (solo ve SUS estados, nunca el panel fábrica)
     ============================================================ */
  function renderConductor() {
    ...
    if (cStep === "inicio") {
      ...
      if (r.estado === "rechazado") {
        v.innerHTML = `<div class="c-wrap"><div class="c-hero">
          <div class="gate-ic err">✕</div><h2>Ingreso no autorizado</h2>
          <p>Portería no autorizó el ingreso de <b>${r.placa}</b>. Acércate a portería o intenta de nuevo.</p>
      ...
      v.innerHTML = `
        <div class="c-wrap">
          ...
          <div class="plate-show"><div class="plate">${r.placa}</div><small>${r.nombre} · CC ${r.cedula}</small></div>
          <div class="tl">${tl}</div>
          ${done ? `
            <div class="cargo-box"><div class="l">Salió con</div>${r.salida_tipo || "—"}${r.salida_detalle ? " · " + r.salida_detalle : ""}</div>
            <div class="c-times">
  ...
  function markArrival(btn) {
    btn.disabled = true;
    btn.innerHTML = '<span class="spin"></span> Obteniendo ubicación…';
  ...
  async function submitConductor(btn) {
    ...
    const cedula = el("fCedula").value.trim();
    const placa = el("fPlaca").value.trim().toUpperCase();
    const motivo = el("fMotivo").value.trim();
    const tipo = el("fTipo").value;
    let bad = false;
    [["fNombre", nombre], ["fCedula", cedula], ["fPlaca", placa]].forEach(([id, val]) => {
      el(id).style.borderColor = val ? "" : "var(--rojo)"; if (!val) bad = true;
    });
    ...
    const rec = {
      id: Store.uuid(),
      folio: "IN-" + String(Date.now()).slice(-5),
      nombre, cedula, placa, motivo: motivo || "—", tipo,
      lat: loc.lat, lng: loc.lng, gps_sim: !!loc.sim,
      estado: "puerta",
      t_puerta: new Date().toISOString(),
      t_ingreso: null, t_salida: null,
      salida_tipo: null, salida_detalle: null, salida_doc: null
    };
    try {
      const saved = await Store.insert(rec);
      myId = saved.id || rec.id;
      localStorage.setItem("cp_myId", myId);
      cStep = "enviado";
      await refresh();
      toast("green", "✅ Aviso enviado", "Portería ya ve tu vehículo en puerta.");
    } catch (e) {
      btn.disabled = false; btn.innerHTML = "Avisar a portería";
      toast("blue", "No se pudo enviar", "Revisa la conexión e inténtalo otra vez.");
    }
  }
  ...
  function renderFabrica() {
    ...
    const cerrado = data.filter(r => r.estado === "cerrado");
    el("tabPuerta").textContent = puerta.length;
    el("tabPlanta").textContent = planta.length;
    el("tabCerrado").textContent = cerrado.length;

    const body = el("fabBody");
    if (tab === "dashboard") { renderDashboard(body, cerrado, puerta, planta); return; }

    const list = tab === "puerta" ? puerta : tab === "planta" ? planta : cerrado;
    if (!list.length) {
      body.innerHTML = `<div class="empty">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="7" width="15" height="10" rx="2"/><path d="M16 10h4l3 3v4h-7z"/><circle cx="5.5" cy="18.5" r="2"/><circle cx="18.5" cy="18.5" r="2"/></svg>
  ...
  function cardHTML(v) {
    ...
    const cargo = v.estado === "cerrado"
      ? `<div class="cargo-box"><div class="l">Salió con</div>${v.salida_tipo || "—"}${v.salida_detalle ? " — " + v.salida_detalle : ""}${v.salida_doc ? " · " + v.salida_doc : ""}</div>` : "";
    return `<div class="vcard${demora ? ' warn' : ''}">
      <div class="vc-top"><span class="plate">${v.placa}</span>
        <span class="st-pill ${st[0]}"><span class="d"></span>${st[1]}${demora ? ' · demora' : ''}</span></div>
      <div class="vc-body">
        <div class="drow"><span class="k">Conductor</span><span class="v">${v.nombre}</span></div>
        <div class="drow"><span class="k">Cédula</span><span class="v">${v.cedula}</span></div>
        <div class="drow"><span class="k">Vehículo</span><span class="v">${v.tipo || "—"}</span></div>
        <div class="drow"><span class="k">Motivo</span><span class="v">${v.motivo || "—"}</span></div>
        <div class="loc-line"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
          ${v.gps_sim ? "Aprox." : "GPS"}: ${(+v.lat).toFixed(4)}, ${(+v.lng).toFixed(4)} · ${distTxt(dist)}
          <a href="${mapUrl}" target="_blank" rel="noopener">Mapa ↗</a></div>
        <div class="times">
  ...
  async function autorizar(id) {
    await Store.update(id, { estado: "planta", t_ingreso: new Date().toISOString() });
    await refresh();
    const v = data.find(x => x.id === id);
    toast("blue", "🏭 Ingreso autorizado", (v ? v.placa : "Vehículo") + " entró a la planta.");
  }
  async function rechazar(id) {
    await Store.update(id, { estado: "rechazado" });
    await refresh();
    toast("amber", "Solicitud rechazada", "El conductor verá el aviso.");
  }
  function openOut(id) {
    outTargetId = id;
    const v = data.find(x => x.id === id);
    el("outSub").textContent = v ? v.placa + " · " + v.nombre : "Cierre";
    el("outDetalle").value = ""; el("outDoc").value = ""; el("outTipo").selectedIndex = 0;
  ...
  async function confirmOut() {
    ...
    await Store.update(outTargetId, {
      estado: "cerrado", t_salida: new Date().toISOString(),
      salida_tipo: el("outTipo").value,
      salida_detalle: el("outDetalle").value.trim(),
      salida_doc: el("outDoc").value.trim()
    });
    const id = outTargetId; closeOut();
    await refresh();
    const v = data.find(x => x.id === id);
    toast("green", "✅ Salida registrada", (v ? v.placa : "Vehículo") + " cerrado correctamente.");
  }

  /* ============================================================
     DASHBOARD (intervalos, demoras, exportación)
     ============================================================ */
  function renderDashboard(body, cerrado, puerta, planta) {
    ...
    const demorasCount = cerrado.filter(r => { const e = tiempos(r).espera; return e != null && e >= C.DEMORA_PUERTA_MIN; }).length
      + puerta.filter(r => diffMin(r.t_puerta, new Date().toISOString()) >= C.DEMORA_PUERTA_MIN).length;

    body.innerHTML = `
      <div class="dash">
        <div class="dash-head">
          <div><h3>Tablero de tiempos</h3><small>Indicadores del proceso en planta</small></div>
          <div class="exp-btns">
            <button class="btn-exp" onclick="App.exportCSV()">⬇️ CSV</button>
            <button class="btn-exp xls" onclick="App.exportXLSX()">⬇️ Excel</button>
          </div>
        </div>
        <div class="dkpis">
        ...
        <div class="tbl-wrap">
          <table class="tbl">
            ...
            <tbody>${cerrado.length ? cerrado.map(rowHTML).join("") : '<tr><td colspan="9" class="tc-empty">Aún no hay registros cerrados para mostrar.</td></tr>'}</tbody>
          </table>
        </div>
      </div>`;
  ...
  function rowHTML(r) {
    const t = tiempos(r);
    const dem = t.espera != null && t.espera >= C.DEMORA_PUERTA_MIN;
    return `<tr>
      <td><b>${r.placa}</b></td><td>${r.nombre}</td>
      <td>${fmtHM(D(r.t_puerta))}</td><td>${fmtHM(D(r.t_ingreso))}</td><td>${fmtHM(D(r.t_salida))}</td>
      <td class="${dem ? 'dem' : ''}">${durTxt(t.espera)}</td><td>${durTxt(t.planta)}</td><td>${durTxt(t.ciclo)}</td>
      <td>${r.salida_tipo || "—"}</td></tr>`;
  }
    ...
    const draw = () => {
      ...
      chart = new Chart(cv.getContext("2d"), {
        ...
        options: {
          ...
          scales: { x: { stacked: false, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { size: 10 } } } }
        }
      });
    };
  ...
  function rowsForExport() {
    return data.filter(r => r.estado !== "rechazado").map(r => {
      const t = tiempos(r);
      return {
        Folio: r.folio || "", Placa: r.placa, Conductor: r.nombre, Cedula: r.cedula,
        Vehiculo: r.tipo || "", Motivo: r.motivo || "", Estado: r.estado,
        ...
        Salio_con: r.salida_tipo || "", Detalle: r.salida_detalle || "", Documento: r.salida_doc || "",
        Lat: r.lat, Lng: r.lng
      };
    });
  }
  ...
  function exportCSV() {
    ...
    if (!rows.length) return toast("blue", "Sin datos", "No hay registros para exportar.");
    ...
    toast("green", "CSV exportado", rows.length + " registros descargados.");
  }
  function exportXLSX() {
    const rows = rowsForExport();
    if (!rows.length) return toast("blue", "Sin datos", "No hay registros para exportar.");
    const go = () => {
      ...
      XLSX.writeFile(wb, `ControlPuerta_${stamp()}.xlsx`);
      toast("green", "Excel exportado", rows.length + " registros descargados.");
    };
    if (window.XLSX) go();
    else {
      const s = document.createElement("script");
      s.src = "https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js";
      s.onload = go; s.onerror = () => toast("blue", "No se pudo cargar Excel", "Usa la exportación CSV."); document.head.appendChild(s);
    }
  }
  ...
  function toast(type, title, msg) {
    ...
    t.innerHTML = `<div class="tic"></div><div><b>${title}</b><span>${msg}</span></div>`;
  ...
  return {
    ...
    setTab, autorizar, rechazar, openOut, closeOut, confirmOut,
    exportCSV, exportXLSX
  };
})();

document.getElementById("btnExit").addEventListener("click", App.exitRole);
document.getElementById("pinInput") && document.getElementById("pinInput").addEventListener("keydown", e => { if (e.key === "Enter") App.checkPin(); });
App.boot();
